// 全国标准省市数据(省 -> 该省地级市)。直辖市按整市处理(cities 为空,不细分区)。
// "国外" 为特殊项,供没有中国省份的客户使用。空数组表示该省级单位作为整体、无需细分到市。
...
export const BUSINESS_LINES = ["国内销售", "外贸"] as const;
export type BusinessLine = (typeof BUSINESS_LINES)[number];

// 把传入的负责范围按官方省市数据清洗:剔除非法省份、剔除该省下不存在的市;cities 为空=整省
export function sanitizeTerritories(
  raw: any
): { province: string; cities: string[] }[] {
  let arr: any = raw;
  ...
  const out: { province: string; cities: string[] }[] = [];
  const seen = new Set<string>();
  for (const t of arr) {
    if (!t || typeof t.province !== "string") continue;
    const prov = t.province;
    if (!(prov in PROVINCE_CITY_MAP) || seen.has(prov)) continue;
    seen.add(prov);
    const validCities = PROVINCE_CITY_MAP[prov];
    let cities = Array.isArray(t.cities)
      ? t.cities.filter((c: any) => typeof c === "string")
      : [];
    cities = validCities.length > 0 ? cities.filter((c: string) => validCities.includes(c)) : [];
    out.push({ province: prov, cities });
  }
  return out;
}
