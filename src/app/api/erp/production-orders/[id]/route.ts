...
import { buildDraftData, getProductionOrderDetail, normalizeDraftInput, ProductionOrderRequestError } from "@/lib/production-orders";
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const user = await getSessionUser();
  if (!user) return NextResponse.json({ error: "未登录" }, { status: 401 });
  if (!canAccessERP(user)) return NextResponse.json({ error: "无权限编辑生产工单" }, { status: 403 });
  const { id } = await params;
  let input;
  try {
