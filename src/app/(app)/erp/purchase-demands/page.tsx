...
export default function PurchaseDemandsPage() {
  ...
  return <PageContainer variant="data" className="space-y-5">
    ...
    <section className="rounded-xl border bg-white p-4"><h2 className="font-medium">新增备货需求</h2><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(260px,2fr)_minmax(120px,0.7fr)_minmax(150px,0.9fr)_minmax(160px,1fr)_minmax(190px,1.1fr)]"><div className="md:col-span-2 xl:col-span-1"><MaterialCombobox materials={materials} value={form.materialId} onChange={(materialId) => setForm({ ...form, materialId })} /></div><input className="min-w-0 rounded border p-2 text-sm" type="number" min="0.01" placeholder="备货数量" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} /><input className="min-w-0 rounded border p-2 text-sm" type="date" value={form.needByDate} onChange={(event) => setForm({ ...form, needByDate: event.target.value })} /><input className="min-w-0 rounded border p-2 text-sm" placeholder="备货用途" value={form.stockPurpose} onChange={(event) => setForm({ ...form, stockPurpose: event.target.value })} /><select className="min-w-0 rounded border p-2 text-sm" value={form.replenishmentReason} onChange={(event) => setForm({ ...form, replenishmentReason: event.target.value })}>{["安全库存补充", "常用物料备货", "长周期物料提前采购", "价格上涨前备货", "供应商停产风险", "售后备件", "临时备货", "其他"].map((value) => <option key={value}>{value}</option>)}</select></div><button onClick={create} className="mt-3 rounded bg-gray-900 px-4 py-2 text-sm text-white">保存备货需求</button></section>
