...
export default function ContractDeleteRequestsPage() {
  ...
  const fetchRequests = () => {
    ...
    fetch(`/api/contract-delete-requests?${params.toString()}`)
      .then((res) => res.json())
  ...
  const handleApprove = async (id: string) => {
    if (!confirm("确认同意删除该合同？同意后合同将被标记删除（数据仍保留，可后续恢复）。")) return;
    setError("");
    const res = await fetch(`/api/contract-delete-requests/${id}/approve`, {
      method: "POST",
  ...
  const handleReject = async (id: string) => {
    setError("");
    const res = await fetch(`/api/contract-delete-requests/${id}/reject`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ approvalRemark: remark[id] || "" }),
    });
  ...
  if (userRole && userRole !== "SUPER_ADMIN") {
    return <p className="text-center py-8 text-red-600">无权查看合同删除审批</p>;
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-900">合同删除审批</h1>
        <select
          value={status}
              ...
              {item.status === "PENDING" && (
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    onChange={(event) => setRemark({ ...remark, [item.id]: event.target.value })}
                    placeholder="审批备注，拒绝时必填"
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm"
                  />
                  <button onClick={() => handleApprove(item.id)} className="px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700">
                    同意删除
                  </button>
                  <button
