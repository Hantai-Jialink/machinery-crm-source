...
const required = [
  ...
  'CANARY_FASTGPT_ROOT_PASSWORD', 'CANARY_FASTGPT_ROOT_KEY', 'CANARY_FASTGPT_TOKEN_KEY', 'CANARY_FASTGPT_FILE_TOKEN_KEY', 'CANARY_FASTGPT_AES_KEY', 'CANARY_FASTGPT_INVOKE_TOKEN_SECRET', 'CANARY_FASTGPT_PLUGIN_TOKEN', 'CANARY_AIPROXY_IMAGE', 'CANARY_AIPROXY_ADMIN_KEY', 'CANARY_AIPROXY_POSTGRES_PASSWORD', 'CANARY_MODEL_UPSTREAM_ENDPOINT', 'CANARY_MODEL_UPSTREAM_API_KEY',
];
...
for (const [name, minimum] of Object.entries({
  CANARY_FASTGPT_ROOT_KEY: 6,
  CANARY_FASTGPT_FILE_TOKEN_KEY: 6,
  CANARY_FASTGPT_AES_KEY: 6,
  CANARY_FASTGPT_INVOKE_TOKEN_SECRET: 32,
  CANARY_FASTGPT_PLUGIN_TOKEN: 32,
})) {
  if (values[name].length < minimum) throw new Error(`${name} must contain at least ${minimum} characters for FastGPT v4.15.2`);
}
const canaryFastGptSecretNames = [
  'CANARY_FASTGPT_ROOT_PASSWORD',
  'CANARY_FASTGPT_ROOT_KEY',
  'CANARY_FASTGPT_TOKEN_KEY',
  'CANARY_FASTGPT_FILE_TOKEN_KEY',
  'CANARY_FASTGPT_AES_KEY',
  'CANARY_FASTGPT_INVOKE_TOKEN_SECRET',
  'CANARY_FASTGPT_PLUGIN_TOKEN',
];
if (new Set(canaryFastGptSecretNames.map((name) => values[name])).size !== canaryFastGptSecretNames.length) {
  throw new Error('Canary FastGPT credentials and purpose-specific secrets must be distinct');
}
