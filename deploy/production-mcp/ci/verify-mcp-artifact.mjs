...
if (!envFile) throw new Error('Usage: node verify-mcp-artifact.mjs <production-env-file>');
...
const archive = values.MCP_IMAGE_ARCHIVE;
const expectedTag = values.MCP_IMAGE;
const proofPath = path.resolve(path.dirname(envFile), values.MCP_IMAGE_IDENTITY_PROOF);
assert.ok(path.isAbsolute(archive) && fs.statSync(archive).isFile(), 'MCP image archive is missing');
const proof = JSON.parse(fs.readFileSync(proofPath, 'utf8'));
const archiveDigest = `sha256:${createHash('sha256').update(fs.readFileSync(archive)).digest('hex')}`;
assert.equal(archiveDigest, proof.archiveSha256, 'MCP image archive SHA256 mismatch');

const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'dachuan-mcp-archive-'));
try {
  ...
  assert.equal(identity.repoTag, proof.repoTag);
  assert.equal(identity.configDigest, proof.configDigest);
  assert.equal(identity.os, 'linux');
  assert.equal(identity.architecture, 'amd64');
} finally {
  fs.rmSync(temporaryDirectory, { recursive: true, force: true });
}
console.log('MCP_IMAGE_ARCHIVE_IDENTITY=PASS');
