...
const configFilename = path.basename(entry.Config);
const configFilenameDigest = configFilename.endsWith('.json') ? configFilename.slice(0, -'.json'.length) : configFilename;
assert.equal(configFilenameDigest, configDigest.slice('sha256:'.length), 'Docker archive Config filename digest mismatch');
