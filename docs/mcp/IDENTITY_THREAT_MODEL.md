...
| 服务 Key 被当成管理员身份 | 服务 Key 不绑定业务用户；三凭证缺一拒绝；旧路径默认关闭 | 必须清理生产旧配置并确认兼容开关为 false |
...
| 两个并发用户串身份 | 身份仅在请求头和 FastGPT AsyncLocalStorage；每次 MCP 建立请求上下文；无用户身份全局变量 | 需在真实 FastGPT 流式和非流式链路压测 |
...
| FastGPT 升级覆盖补丁 | 精确提交校验、独立 patch、自动 apply/rollback、固定自定义镜像标签 | 每次升级必须重新审查和运行并发测试，禁止漂移到 latest |
